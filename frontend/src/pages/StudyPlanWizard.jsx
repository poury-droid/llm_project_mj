...
export default function StudyPlanWizard({ applicationMode = false }) {
  ...
  const [settings, setSettings] = useState({ weekday: 2, weekend: 4, excluded: [], date: "", intensity: "normal" });
  ...
  return <section>
    ...
    <form className="panel study-wizard" onSubmit={create}>
      ...
      {step === 3 && <div className="wizard-section"><h2>공부 가능시간을 입력해주세요</h2><div className="form-grid"><label>평일 공부 가능시간<input type="number" min="0" value={settings.weekday} onChange={(e) => setSettings({ ...settings, weekday: e.target.value })} />시간</label><label>주말 공부 가능시간<input type="number" min="0" value={settings.weekend} onChange={(e) => setSettings({ ...settings, weekend: e.target.value })} />시간</label></div><h3>공부하지 못하는 날짜</h3><div className="inline-form compact-form"><input type="date" value={settings.date} onChange={(e) => setSettings({ ...settings, date: e.target.value })} /><button type="button" className="button secondary" onClick={addDate}>날짜 추가</button></div><div className="tags">{settings.excluded.map((date) => <span key={date}>{date}<button type="button" onClick={() => setSettings({ ...settings, excluded: settings.excluded.filter((value) => value !== date) })}>×</button></span>)}</div><h3>계획 강도</h3><div className="choice-grid compact">{[["relaxed", "여유롭게"], ["normal", "보통"], ["intensive", "집중적으로"]].map(([value, label]) => <button type="button" className={settings.intensity === value ? "choice active" : "choice"} key={value} onClick={() => setSettings({ ...settings, intensity: value })}>{label}</button>)}</div><details><summary>상세 설정</summary><p className="muted">기본 설정으로 시험일까지 가능한 시간에 맞춰 자동 배분합니다.</p></details><div className="wizard-actions"><button type="button" className="button secondary" onClick={() => setStep(2)}>이전</button><button className="button" type="submit">공부계획 만들기</button></div></div>}
    </form>
  </section>;
}
